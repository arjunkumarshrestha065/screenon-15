const DEFAULT_SERVER_URL = 'http://ads.asianpaints.com.np';
...
function ensureAdSlots() {
  ...
  if (!el('adSlotA') || !el('adSlotB')) {
    stage.innerHTML = '<div id="adSlotA" class="active"></div><div id="adSlotB"></div>';
    activeAdSlot = 'A';
  }
  return stage;
}
...
function getActiveSlot() {
  return el(activeAdSlot === 'A' ? 'adSlotA' : 'adSlotB');
}

function getNextSlot() {
  return el(activeAdSlot === 'A' ? 'adSlotB' : 'adSlotA');
}
...
function switchAdSlots(nextSlot) {
  ...
  if (activeSlot && activeSlot !== nextSlot) {
    ...
    setTimeout(function () {
      try {
        const oldVideo = activeSlot.querySelector('video');
        if (oldVideo) {
          oldVideo.pause();
  ...
  activeAdSlot = nextSlot && nextSlot.id === 'adSlotB' ? 'B' : 'A';
  setTimeout(hideTransitionLayer, 120);
}

function playNext() {
  const stage = ensureAdSlots();
  if (!stage) return;
  ...
  if (!playlist.length) {
    stage.innerHTML = '<div style="height:100vh;display:flex;align-items:center;justify-content:center;background:#020617;color:white;font:30px Arial;text-align:center;white-space:pre-line;padding:25px;box-sizing:border-box">No downloaded ads available</div>';
  ...
  const url = item.local_url || item.full_url || item.file_url;
  const rawDuration = item.duration;
  const duration = (rawDuration === undefined || rawDuration === null || rawDuration === '') ? 0 : Number(rawDuration);
  const hasCustomDuration = !isNaN(duration) && duration > 0;
  const effect = randomFx();
  ...
  if (item.file_type === 'video') {
    nextSlot.innerHTML =
      '<video ' +
      'id="adVideo_' + Date.now() + '" ' +
      'class="ad-item ' + effect + '" ' +
      'src="' + url + '" ' +
      'autoplay ' +
      'muted ' +
      'playsinline ' +
      ...
      'style="width:100vw;height:100vh;object-fit:contain;background:#000;pointer-events:none;">' +
      '</video>';

    const video = nextSlot.querySelector('video');
    if (!video) {
      hideTransitionLayer();
      playTimer = setTimeout(playNext, 2000);
      return;
    ...
    video.oncanplay = finishSwitch;
    video.onloadeddata = function () {
      setTimeout(finishSwitch, 60);
    };
    ...
    // Fallback if oncanplay is not fired on some Android TV WebView versions.
    setTimeout(function () {
      if (!switched && video.readyState >= 2) finishSwitch();
    }, 1200);

    // Video duration 0 means play full video until video.onended.
    ...
    // Image has no natural ending. Image duration 0 or blank means default 10 seconds.
    playTimer = setTimeout(playNext, (hasCustomDuration ? duration : 10) * 1000);
  }
}
...
// Admin one-by-one client player update checker
// This checks only this client's queued update job and reports individual progress.
(function(){
  const CLIENT_ONE_BY_ONE_UPDATE_CHECK_MS = 60 * 1000;
  let clientUpdateBusy = false;
  let clientUpdateResolvers = {};

  function clientResolveUpdateUrl(path){
    if(!path) return '';
    if(/^https?:\/\//i.test(path)) return path;
    return (serverUrl || '').replace(/\/+$/, '') + '/' + String(path).replace(/^\/+/, '');
  }

  window.onClientSpecificWebUpdateComplete = window.onWebUpdateComplete;
  const originalOnWebUpdateComplete = window.onWebUpdateComplete;
  window.onWebUpdateComplete = function(updateId, result){
    if(clientUpdateResolvers[updateId]){
      clientUpdateResolvers[updateId](result);
      delete clientUpdateResolvers[updateId];
      return;
    }
    if(typeof originalOnWebUpdateComplete === 'function'){
      originalOnWebUpdateComplete(updateId, result);
    }
  };

  async function reportClientUpdateProgress(jobId, progress, status, file, note){
    try{
      await fetch(serverUrl.replace(/\/+$/, '') + '/api/client/web-update/progress',{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({
          token:token,
  ...
  async function checkClientSpecificUpdate(){
    ...
    try{
      ...
      const res = await fetch(serverUrl.replace(/\/+$/, '') + '/api/client/web-update/job/' + token, { cache:'no-store' });
      if(res.status === 401){
        localStorage.removeItem('token');
        token='';
        setTimeout(init,1500);
      ...
      if(!res.ok){ clientUpdateBusy = false; return; }
      const job = await res.json();
      if(!job.hasUpdate){ clientUpdateBusy = false; return; }

      const localVersion = Number(AndroidBridge.getWebVersion() || 1);
      const targetVersion = Number(job.version || 1);
      if(targetVersion <= localVersion){
        await reportClientUpdateProgress(job.jobId,100,'completed','Already updated','Local version already equal or higher');
        clientUpdateBusy = false;
        return;
      }

      overlay('Updating player files v' + targetVersion + '...');
      await reportClientUpdateProgress(job.jobId,5,'downloading','Preparing','Client accepted update job');

      const updateId = 'client_web_' + Date.now() + '_' + Math.random().toString(36).slice(2);
      const result = await new Promise(function(resolve){
        clientUpdateResolvers[updateId] = resolve;
        AndroidBridge.downloadWebUpdateAsync(updateId, targetVersion, clientResolveUpdateUrl(job.playerHtml), clientResolveUpdateUrl(job.playerJs));
      });

      if(result && result.success){
        await reportClientUpdateProgress(job.jobId,100,'completed','Completed','Player files updated');
        overlay('Player updated. Reloading...');
        setTimeout(function(){ AndroidBridge.reloadPlayer(); },1200);
      }else{
        await reportClientUpdateProgress(job.jobId,0,'failed','Failed',(result && result.error) || 'unknown error');
        overlay('Player update failed');
        setTimeout(function(){ overlay(''); },4000);
      }
    }catch(e){} finally {
      clientUpdateBusy=false;
  ...
  window.checkClientSpecificUpdate = checkClientSpecificUpdate;
  window.addEventListener('load',function(){
    setTimeout(checkClientSpecificUpdate,10000);
    setInterval(checkClientSpecificUpdate,CLIENT_ONE_BY_ONE_UPDATE_CHECK_MS);
  });
})();
